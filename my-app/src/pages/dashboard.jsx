...
function DashboardPage() {
    ...
    const fetchUserAndRooms = async () => {
      ...
      try {
        ...
        try {
          ...
          const roomData = await api.viewRooms(token);
          ...
          setError(err.message || 'Failed to fetch rooms. Showing default room.');
  ...
  const createPublicRoom = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const data = await api.createPublicRoom(token, { name: publicRoomName });
  ...
  const createPrivateRoom = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const data = await api.createPrivateRoom(token, { name: privateRoomName });
      setRooms([...rooms, { ...data.room, type: 'private', token: data.room.token }]);
      setChannels({ ...channels, [data.room.name]: ['welcome', 'chat'] });
      setActiveRoom(data.room.name);
      setActiveChannel('welcome');
      setShowModal(false);
  ...
  const joinPrivateRoom = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const data = await api.joinPrivateRoom(token, privateRoomToken);
      setRooms([...rooms, { ...data, type: 'private', token: privateRoomToken }]);
      ...
      setError('No room found with that token');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Modal component for room actions
  const RoomModal = () => (
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {loading ? (
              ...
              <div className="flex flex-col space-y-4">
                {/* Token Display for Private Rooms */}
                  ...
                  <div className="p-3 bg-zinc-800 rounded-md border border-zinc-700">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-sm text-zinc-400">{rooms.find(r => r.name === activeRoom)?.token}</span>
                      </div>
                      <button
                        onClick={() => navigator.clipboard.write(rooms.find(r => r.name === activeRoom)?.token)}
                        className="text-zinc-400 hover:text-zinc-200"
                        title="Copy Token"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
                ...
                <div className="text-center py-6">
                  ...
                  <h2 className="text-2xl font-bold text-zinc-200">Welcome to {activeChannel ? `#${activeChannel}` : 'SodaClub'}</h2>
                  <p className="text-zinc-400 mt-2">
                    {activeRoom ? `This is the beginning of the ${activeChannel} channel in ${activeRoom}.` : 'Join a room to start chatting!'}
                  </p>
                </div>
                ...
                {activeChannel && (
                  <div className="border-t border-zinc-800 pt-4 mt-4">
                    <div className="flex items-start mb-6">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center">
                          <span className="font-semibold text-zinc-200 mr-2">SodaBot</span>
                          <span className="text-xs text-zinc-500">Today at 12:00 PM</span>
                        </div>
                        <div className="mt-1 text-zinc-300">
                          Welcome to {activeRoom}! This is the {activeChannel} channel. Share something with the community or check out what others are saying.
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="p-4 border-t border-zinc-800">
            <div className="relative bg-zinc-800 rounded-md border border-zinc-700">
              <input
                ...
                className="w-full bg-transparent rounded-md px-4 py-3 text-zinc-200 border-none focus:outline-none focus:ring-1 focus:ring-zinc-600"
              />
              <div className="absolute right-2 top-2 flex items-center space-x-1">
                <button className="w-8 h-8 flex items-center justify-center text-zinc-400 hover:text-zinc-200 rounded-md transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
                ...
                <button className="ml-1 px-3 py-1 bg-gradient-to-r from-zinc-700 to-zinc-600 rounded-md text-zinc-200 hover:shadow-md transition-all duration-300">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
        ...
        <div className="w-60 bg-zinc-900 border-l border-zinc-800 hidden md:block">
            ...
            <div className="p-2 rounded-md hover:bg-zinc-800 flex items-center cursor-pointer transition-all duration-300">
              ...
              <div className="text-sm flex items-center">
                ...
                <div className="ml-1 inline-flex items-center justify-center">
                  <div className="flex items-center bg-zinc-800 rounded px-1.5 py-0.5">
                    ...
                    <span className="text-xs font-medium text-amber-200">{coins}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="px-4 py-2">
            <div className="text-xs font-semibold text-zinc-400 px-2 py-1 mt-2">ROOM INFO</div>
            {activeRoom && (
              <div className="mt-2 px-2">
                <div className="text-xs text-zinc-400 mb-1">Room Type:</div>
                <div className="text-sm text-zinc-300 mb-2">
                  {rooms.find(r => r.name === activeRoom)?.type === 'private' ? (
                    <div className="flex items-center">
                ...
                <div className="text-sm text-zinc-300">April 28, 2025</div>
              </div>
            )}
          </div>
        </div>
